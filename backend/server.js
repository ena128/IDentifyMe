...
// Koristi MONGODB_URI iz okruženja ili lokalnu bazu
...
app.post('/verify', upload.fields([{ name: 'idImage' }, { name: 'selfieImage' }]), async (req, res) => {
    ...
    try {
        ...
        // Provjera dokumenta
        const idKeywords = ["identity", "card", "hercegovina", "republic", "birth", "prezime", "ime", "datum"];
        const hasIdKeywords = idKeywords.some(keyword => cleanedText.toLowerCase().includes(keyword));

        if (!hasIdKeywords) {
            resultText = "No ID found ❌: Please insert your ID photo.";
        ...
        // Šaljemo STVARNI status uspjeha
