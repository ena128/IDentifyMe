...
// Produkcijski ili lokalni MongoDB URI
...
app.post('/verify', upload.fields([{ name: 'idImage' }, { name: 'selfieImage' }]), async (req, res) => {
    ...
    try {
        ...
        // 3. Provjera da li je dokument prepoznat
        ...
        if (isSamePerson && isAdult) {
            ...
            resultText = `Verification Failed ❌: Person is under 18 years old (${age})`;
        } else {
            resultText = "Verification Failed ❌: Faces don't match and person is under 18";
        }

        // 6. Spašavanje u bazu podataka
        ...
        // SLANJE ODGOVORA - success sada zavisi od biometrijskih uslova
        res.json({ 
